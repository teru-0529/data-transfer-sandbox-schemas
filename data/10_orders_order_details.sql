...
-- Create Table
DROP TABLE IF EXISTS orders.order_details CASCADE;
...
-- Set Table Comment
COMMENT ON TABLE orders.order_details IS '受注明細';

-- Set Column Comment
COMMENT ON COLUMN orders.order_details.order_no IS '受注番号';
COMMENT ON COLUMN orders.order_details.product_id IS '商品ID';
COMMENT ON COLUMN orders.order_details.receiving_quantity IS '受注数';
COMMENT ON COLUMN orders.order_details.shipping_quantity IS '出荷数';
COMMENT ON COLUMN orders.order_details.cancel_quantity IS 'キャンセル数';
COMMENT ON COLUMN orders.order_details.remaining_quantity IS '受注残数';
COMMENT ON COLUMN orders.order_details.sellling_price IS '販売単価';
COMMENT ON COLUMN orders.order_details.cost_price IS '商品原価';
COMMENT ON COLUMN orders.order_details.order_status IS '受注ステータス';
COMMENT ON COLUMN orders.order_details.created_at IS '作成日時';
COMMENT ON COLUMN orders.order_details.updated_at IS '更新日時';
COMMENT ON COLUMN orders.order_details.created_by IS '作成者';
COMMENT ON COLUMN orders.order_details.updated_by IS '更新者';

-- Set PK Constraint
ALTER TABLE orders.order_details ADD PRIMARY KEY (
  order_no,
  product_id
);
...
CREATE TRIGGER set_updated_at
  BEFORE UPDATE
  ON orders.order_details
  FOR EACH ROW
EXECUTE PROCEDURE set_updated_at();

-- Create 'append_history' Function
DROP FUNCTION IF EXISTS orders.order_details_audit();
CREATE OR REPLACE FUNCTION orders.order_details_audit() RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER audit
  ...
  ON orders.order_details
  FOR EACH ROW
EXECUTE PROCEDURE orders.order_details_audit();
